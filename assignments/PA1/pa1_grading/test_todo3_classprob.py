...
if __name__ == '__main__':
    print("Public Accuracy Test Case 3 - Tests function 'compute_class_prob' in NaiveBayesClassifier")
    with HiddenPrints():
        ...
        # Expected output: with N=2 classes, d=4 documents:
        # P(c_0) = (1 + 2)/(2 + 4) = 0.5
        # P(c_1) = (1 + 2)/(2 + 4) = 0.5
        expected = np.array([0.39665653, 0.60334347])
        
        # Initialize the classifier
        nb_classifier = NaiveBayesClassifier()
        
        # Call the method directly
        student_result=nb_classifier.compute_class_prob(train_bow, train_labels)
        # student_result = nb_classifier.compute_class_prob(X, y)
    
    # Check if results match within a small tolerance
    if np.allclose(student_result, expected, rtol=1e-5):
        print(True)
    else:
        print("compute_class_prob does not produce the expected result.")
