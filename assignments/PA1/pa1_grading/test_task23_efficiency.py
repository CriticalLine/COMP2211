...
if __name__ == '__main__':
    print("Public Efficiency Test Case 7 - Tests function 'find_ambiguous_words'")
    try:
        with HiddenPrints():
            ...
            pos_words, pos_scores = find_ambiguous_words(log_odds, vocab, top_k=-1)
            end_time = time.time()
            print(pos_words, pos_scores)
            execution_time = end_time - start_time
            
            # Reasonable threshold for a vectorized implementation
            time_threshold = 0.0005  # seconds 0.1
        ...
        if execution_time <= time_threshold:
            ...
            print(False)
            print(f"find_ambiguous_words is too slow. Execution time: {execution_time:.6f} seconds (threshold: {time_threshold} seconds)")
